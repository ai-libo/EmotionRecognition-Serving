...
if __name__ == '__main__':
    ...
    pred3d_x = pred3d_x.reshape([1 ,27648] )
    print ("#######hape#######",pred3d_x.shape)

    #记个时
    start_time = time.time()
    #建立连接
    channel = implementations.insecure_channel("211.159.158.25", 8000)
    ...
    print (">>>>>>>>>>>>>>>>>>",request)
    keepprob = np.array([1.])
    request.inputs["input"].ParseFromString(tf.contrib.util.make_tensor_proto(pred3d_x, dtype=dtypes.float32, shape=[1, 27648]).SerializeToString())
