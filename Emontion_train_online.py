...
# tf Graph 输入
x = tf.placeholder(tf.float32, [None, 27648],name='x')
x2 = tf.reshape(x,[-1,96,96,3])
print (x2.shape)
#x2 = tf.placeholder(tf.float32, [None, 96, 96, 1],name='x2')
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    ...
    with tf.gfile.FastGFile(logdir+'Emontion_cnn-graph.pb', mode='wb') as f:
        f.write(constant_graph.SerializeToString())
    '''
    #graph = convert_variables_to_constants(sess, sess.graph_def, ["out2"]) #out为保存网络的最后输出节点名称  
    #tf.train.write_graph(graph, '.', 'model/Emontion_cnn-graph.pb', as_text=False)  
    

    export_path = 'model2/emontion_model'
    ...
    # y 为最终需要的输出结果tensor 
    outputs = {'output' : tf.saved_model.utils.build_tensor_info(predict_label)}
